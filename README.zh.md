...
1. 烧录后给设备上电。默认密码固定，首次登录不需要查看串口输出。
2. 连接 WiFi 热点 `SMS-Forwarder-Setup`，密码 `12345678`。
3. 在浏览器打开 `http://192.168.4.1`。
4. 使用用户名 `admin`、密码 `admin1234` 登录。
...
- SIM 短信索引 `0` 有效（例如 `+CMTI: "SM",0`）。自动通知、手动检查和启动扫描均包含此索引；直接上报的 `+CMT` 短信没有 SIM 存储索引，不会触发删除索引 `0`。
...
- 调制解调器正在收发短信或执行其他 AT 指令时，网络和数据维护会延后，以 1 秒间隔等待空闲后重试。`BUSY` 不会使已知数据状态失效，也不代表数据切换失败；重试时按当前数据策略执行。
- HTTPS 会验证证书和主机名，失败时不会退回不安全模式。私有 CA 配置和系统时间要求见安全说明。配置接口不再返回已保存凭据，表单中可选择保持、替换或清除。
- 推送失败日志包含渠道、失败阶段、耗时及可获取的 HTTP/TLS 错误码，不记录响应正文或凭据。HTTP `-1`（`connect_failed`）可能表示 DNS、TCP 或 TLS 建连失败，不一定是服务器拒绝，且不再误报为响应过大。TLS 配置、读取超时、响应超限、JSON 无效及服务端拒绝会分别报告；超时设置和证书校验保持不变。
